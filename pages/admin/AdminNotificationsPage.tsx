...
const AdminNotificationsPage: React.FC = () => {
    ...
    return (
        <div className="space-y-8">
            <div>
                <h1 className="text-3xl font-extrabold text-admin-heading tracking-tight">Broadcast Notifications</h1>
                <p className="text-admin-light/80 mt-1">Send a notification to all registered users.</p>
            </div>

            {/* Form Section */}
            <form onSubmit={handleSendNotification} className="bg-admin-surface-glass border border-admin-border backdrop-blur-2xl p-8 rounded-2xl shadow-xl shadow-black/10 space-y-6 max-w-3xl">
                <div>
                    ...
                    <textarea
                        ...
                        required
                        className="w-full p-3 bg-admin-surface text-admin-heading border border-admin-border rounded-lg min-h-[100px] focus:ring-2 focus:ring-admin-accent focus:border-admin-accent transition"
                    />
                </div>
                <div>
                    ...
                    <p className="text-sm text-admin-light/80 mb-2">
                        An internal path for the user to navigate to (e.g., <code className="bg-admin-surface px-1.5 py-0.5 rounded-sm">/services</code>). Defaults to <code className="bg-admin-surface px-1.5 py-0.5 rounded-sm">/profile</code> if empty.
                    </p>
                    <input
                        id="notif-link"
                        type="text"
                        ...
                        placeholder="/profile"
                        className="w-full p-3 bg-admin-surface text-admin-heading border border-admin-border rounded-lg focus:ring-2 focus:ring-admin-accent focus:border-admin-accent transition"
                    />
                </div>
                ...
                <div className="flex items-center justify-end gap-4 border-t border-admin-border pt-6">
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="flex items-center gap-2 px-6 py-3 text-base font-bold text-white bg-admin-accent rounded-xl shadow-lg hover:brightness-110 disabled:bg-slate-400 focus:outline-none focus:ring-4 focus:ring-admin-accent/50 transition-all"
                    >
            ...
            {/* History Section */}
            <div className="bg-admin-surface-glass border border-admin-border backdrop-blur-2xl p-8 rounded-2xl shadow-xl shadow-black/10">
                 <h2 className="text-2xl font-bold text-admin-heading mb-4">Sent History</h2>
                 <div className="space-y-4">
                    {historyLoading ? (
                        ...
                        sentHistory.map((notif, index) => (
                            <div key={index} className="p-4 bg-admin-surface border border-admin-border rounded-lg">
                                ...
                                <div className="flex justify-between items-center mt-2 text-xs">
                                    ...
                                    <span className="text-admin-light/60">Link: <code className="font-mono bg-admin-bg px-1.5 py-0.5 rounded-sm">{notif.link}</code></span>
                                </div>
                            </div>
                        ))
                    )}
