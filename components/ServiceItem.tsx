...
const ServiceItem: React.FC<{ service: Service; onClick: () => void }> = ({ service, onClick }) => {
  ...
  return (
    ...
    >
      ...
      <div className={`flex items-center justify-center w-20 h-20 rounded-full ${bg} transition-all duration-300 group-hover:scale-110 group-hover:shadow-lg mt-auto`}>
        <div className={text}>
            <IconMap iconName={service.icon_name} className="h-12 w-12" />
        </div>
      </div>
      <span className="text-sm font-semibold text-slate-700 tracking-tight mt-auto">{service.name}</span>
    </div>
  );
};
