  ...
  const handleMarkAsRead = async (id: number | 'all') => {
      try {
          ...
          if (id === 'all') {
              ...
              query = supabase.from('notifications').update(updatePayload).in('id', allUnreadIds);
          } else {
              query = supabase.from('notifications').update(updatePayload).eq('id', id);
          }
          ...
          if (error) {
              ...
              fetchNotifications(); // Refetch notifications on success
          }
      } catch (error) {
    ...
    <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-lg sticky top-0 z-30 border-b border-slate-200/80 dark:border-slate-800/80">
      <div className="max-w-7xl mx-auto flex justify-between items-center p-4">
        <Link to="/" className="flex items-center gap-2">
           <span className="font-bold text-xl sm:text-2xl text-slate-800 dark:text-slate-200">Document</span>
           <span className="font-bold text-xl sm:text-2xl text-cyan-500">mitra.</span>
        </Link>
